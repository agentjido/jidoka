...
| Idempotency | Per-operation policy that tells the runtime how to treat repeated effects. Valid values: `:pure`, `:idempotent`, `:dedupe`, `:reconcile`, `:unsafe_once`. `:unsafe_once` requires a matching operation control. | [`Jidoka.Agent.Spec.Operation`](`Jidoka.Agent.Spec.Operation`) | [Controls](controls.md) |
...
| LLMDecision | Typed decision returned by an LLM effect. Either `:final` (with content and optional structured result) or `:operation` (with name and arguments). | [`Jidoka.Effect.LLMDecision`](`Jidoka.Effect.LLMDecision`) | [Live LLM Tool Loop](live-llm-tool-loop.md) |
